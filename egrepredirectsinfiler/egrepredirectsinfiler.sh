...
wp_g1="/mnt/projetos/deploy-fe/wordpress_legado/httpd-rewrite-all-jornalismo.conf" # URLs legado do worpress com o  ermo "platb" na URI
...
USAGE_MSG="
Usage: $(basename "$0") <Text String> <Project Name>
    OPTIONS:
    <Text String>   text string to execute egrep in filer projects
    <Project Name>  [$IBlue gshow$NC |$IBlue g1$NC |$IBlue ge$NC |$IBlue gsat3$NC |$IBlue home$NC |$IBlue redeglobo$NC |$IBlue
                      casaejardim$NC |$IBlue assine$NC |$IBlue gnt$NC |$IBlue vignette$NC |$IBlue wpg1$NC |$IBlue
                      vogue$NC |$IBlue casavogue$NC |$IBlue multishow$NC |$IBlue techtudo$NC |$IBlue
                      revistaquem$NC |$IBlue centraldeajuda$NC |$IBlue revistagalileu$NC |$IBlue
                      revistagloborural$NC |$IBlue revistapegn$NC ]
...
# Main program
text_string="${PARAMETERS[0]}"
project_chosen "${PARAMETERS[1]}"
echo -e "\n"$IYellow"ssh -t $LOGNAME@$filer_server egrep -ni --color=always $text_string $redirect_filer_path\n$NC"
...
ssh -t "$LOGNAME"@"$filer_server" egrep -ni --color=always "$text_string" "$redirect_filer_path"